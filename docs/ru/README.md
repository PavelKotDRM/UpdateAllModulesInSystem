...
- Выбор отдельных пакетов применяется там, где менеджер поддерживает адресное обновление; `pacman`, MSYS2, `apk`, `xbps` и `emerge` явно показывают полное системное обновление без чекбоксов пакетов.
...
- кнопка отмены, которая останавливает очередь и завершает уже запущенные деревья процессов;
...
- раскрывающиеся блоки с чекбоксами отдельных пакетов там, где поддерживается адресное обновление; для полного системного обновления показывается соответствующее уведомление;
...
- Для `winget` и `choco` используется специализированный разбор вывода при проверке обновлений; строки без фактического повышения версии отбрасываются. Для `winget` дополнительно проверяется, не установлена ли доступная версия под другим идентификатором пакета. Успешная команда считается достаточным подтверждением: некоторые установщики, включая Unity, сохраняют старую запись версии при немедленной повторной проверке.
...
- Способ установки Node.js определяется по доступным менеджерам, пути `node` и записи MSI в реестре Windows. Поддерживаются `nvm`, `fnm`, `winget`, Homebrew и официальный MSI. В Unix `nvm` загружается из `nvm.sh` в дочернем процессе Bash, а установленная версия назначается alias `default`. Для MSI установщик нужной версии загружается с `nodejs.org` и запускается через `msiexec /passive /norestart`.
- Для DNF/Yum, zypper и snap используются формато-ориентированные парсеры; отдельные Linux-менеджеры с полным обновлением системы не предлагают выбор пакетов.
- Выбранные пакеты передаются адресным командам обновления; `pacman`, MSYS2, `apk`, `xbps` и `emerge` выполняют транзакцию целиком и показывают это в интерфейсе.
...
- Для расширений редакторов обновление каждого выбранного расширения выполняется независимо; если отдельное расширение недоступно в каталоге, остальные всё равно обновляются, а итоговая ошибка перечисляет проблемные расширения.
...
## Чеклист исправлений по результатам ревью

- [x] Учитывать выбор отдельных пакетов для адресных обновляторов и явно отмечать полное обновление системы.
- [x] Уважать `--yes` при обновлении через `apt-get` и MSYS2.
- [x] Показывать ошибки проверки обновлений вместо статуса «Актуален».
- [x] При отмене завершать деревья процессов обновляторов и показывать ошибки завершения.
- [x] Обновлять Node.js через стандартный shell-скрипт `nvm` в Unix.
- [x] Синхронизировать выбор модулей и пакетов и сохранять пустой выбор.
- [x] Правильно разбирать версии в выводе pacman, DNF/Yum, zypper, snap и rustup.
- [x] Разрешать повышение прав для установленных привилегированных модулей при фильтре `--only`.
...
- Для оставшихся Linux-менеджеров (`pacman`, `apk`, `xbps`, `emerge`) проверка опирается на эвристический разбор текстового вывода, поскольку стабильный машинный формат доступен не во всех поддерживаемых версиях.
